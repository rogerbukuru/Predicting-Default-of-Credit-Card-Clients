...
for(k in 1:1){
 ...
 vae_mse[12] = vae_stats["loss"]
 vae_result[[12]]$vae_model = train_vae_model$vae_model
 vae_result[[12]]$input_layer = train_vae_model$input_layer
 vae_result[[12]]$z = train_vae_model$z
}
...
## Autoencoders+K-Means Customer Segments
...
kmeans_result_ae <- kmeans(encoded_points_ae, centers = 2, nstart = 25)
...
legend_position <- s3d_ae$xyz.convert(3.5, -1.0, 0.0)
...
##Variational Autoencoder+K-Means Customer Segments
...
ggplot(wcss_df_vae, aes(x = Clusters, y = WCSS)) +
  ...
  scale_x_continuous(breaks = 1:10) + # Ensure integer scale for clusters
  theme_minimal() +
  labs(title = "Elbow Method for Determining Optimal Number of Clusters - Variational Autoencoder",
       x = "Number of Clusters",
       y = "Within-Cluster Sum of Squares (WCSS)")
...
# tune_range_pca <- list(gamma = 2^(seq(-15, 3, by = 2)), cost = 100^(seq(-5, 15, by = 2)))
# 
...
objects_to_keep <- c("credit_default_data", 
                     ...
                     "test_data_pca_scores",
                     "svm_test_predict_ae",
                     "training_encoded_points_ae_12D",
                     "testing_encoded_points_ae_12D"
                     )
rm(list = setdiff(ls(), objects_to_keep))
...
## PCA Customer Segement SVM Predictions
...
rm(list = setdiff(ls(), objects_to_keep))
# Let's assume 'data_vpca' contains your features transformed by Vpca and 'clusters' is a vector of cluster assignments from k-means
...
svm_models_pca <- list()
performance_metrics_pca <- data.frame(Cluster = integer(),
                                     Accuracy = numeric(),
                                     Recall = numeric(),
                                     Precision = numeric(),
                                     F1_Score = numeric(),
...
training_data_smote_with_clusters_pca = training_data_smote%>%
  mutate(Cluster = clusters_pca)
training_labels_pca = training_data_smote_with_clusters_pca$DEFAULT_PAYMENT_NEXT_MONTH
training_pca_scores_with_clusters = pca_scores%>%
  mutate(Cluster = clusters_pca)


kmeans_test_data_result_pca <- kmeans(test_data_pca_scores[,13], centers = 3, nstart = 25)
test_data_clusters_pca = kmeans_test_data_result_pca$cluster
testing_pca_scores_with_clusters = test_data_pca_scores%>%
  mutate(Cluster = test_data_clusters_pca)
testing_data_with_clusters_pca = testing_data%>%
  mutate(Cluster = test_data_clusters_pca)
testing_labels_pca = testing_data_with_clusters_pca$DEFAULT_PAYMENT_NEXT_MONTH


for (k in unique(clusters_pca)) {
  # Subset the data for cluster 'k'
  cluster_training_data_pca <- training_pca_scores_with_clusters[clusters_pca == k, ]
  cluster_training_labels_pca <- training_labels_pca[clusters_pca == k]
  cluster_test_labels_pca <- testing_labels_pca[test_data_clusters_pca == k]
  
  # Train the SVM model
  svm_models_pca[[k]] <- svm(x = cluster_training_data_pca[, -c(14:27)], y = cluster_training_labels_pca, kernel = "radial", cost = 100, gamma = 0.1)
  
  # Make predictions
  cluster_test_data_pca = testing_pca_scores_with_clusters[test_data_clusters_pca == k, ]
  predictions <- predict(svm_models_pca[[k]], cluster_test_data_pca[, -c(14:27)])
  
  # Calculate performance metrics
  cm <- confusionMatrix(predictions, as.factor(cluster_test_labels_pca))
  ...
  # Calculate AUC
  roc_curve <- roc(response = as.numeric(as.factor(cluster_test_labels_pca)), predictor = as.numeric(predictions))
  auc_value <- auc(roc_curve)
  
  # Store the performance metrics
  performance_metrics_pca <- rbind(performance_metrics_pca, data.frame(Cluster = k,
                                                                     Accuracy = accuracy,
                                                                     Recall = recall,
                                                                     Precision = precision,
                                                                     F1_Score = F1_score,
...
print(performance_metrics_pca)
```


## AE Customer Segement SVM Predictions
...
svm_models_ae <- list()
performance_metrics_ae <- data.frame(Cluster = integer(),
                                  Accuracy = numeric(),
                                  Recall = numeric(),
                                  Precision = numeric(),
                                  F1_Score = numeric(),
...
# Loop through each cluster and train an SVM model
clusters_ae = kmeans_result_ae$cluster
training_data_smote_with_clusters_ae = training_data_smote%>%
                                   mutate(Cluster = clusters_ae)
training_labels_ae = training_data_smote_with_clusters_ae$DEFAULT_PAYMENT_NEXT_MONTH
training_encoded_points_ae_12D_with_clusters = training_encoded_points_ae_12D%>%
                                                mutate(Cluster = clusters_ae)


kmeans_test_data_result_ae <- kmeans(testing_encoded_points_ae_12D, centers = 2, nstart = 25)
test_data_clusters_ae = kmeans_test_data_result_ae$cluster
testing_encoded_points_ae_12D_with_clusters = testing_encoded_points_ae_12D%>%
                                  mutate(Cluster = test_data_clusters_ae)
testing_data_with_clusters_ae = testing_data%>%
                               mutate(Cluster = test_data_clusters_ae)
testing_labels_ae = testing_data_with_clusters_ae$DEFAULT_PAYMENT_NEXT_MONTH


for (k in unique(clusters_ae)) {
  # Subset the data for cluster 'k'
  cluster_training_data_ae <- training_encoded_points_ae_12D_with_clusters[clusters_ae == k, ]
  cluster_training_labels_ae <- training_labels_ae[clusters_ae == k]
  cluster_test_labels_ae <- testing_labels_ae[test_data_clusters_ae == k]
  
  # Train the SVM model
  svm_models_ae[[k]] <- svm(x = cluster_training_data_ae[, -13], y = cluster_training_labels_ae, kernel = "radial", cost = 100, gamma = 0.1)
  
  # Make predictions
  cluster_test_data_ae = testing_encoded_points_ae_12D_with_clusters[test_data_clusters_ae == k, ]
  predictions <- predict(svm_models_ae[[k]], cluster_test_data_ae[, -13])
  
  # Calculate performance metrics
  cm <- confusionMatrix(predictions, as.factor(cluster_test_labels_ae))
  accuracy <- cm$overall['Accuracy']
  recall <- cm$byClass['Sensitivity']
  precision <- cm$byClass['Pos Pred Value']
  F1_score <- 2 * (precision * recall) / (precision + recall)
  ...
  # Calculate AUC
  roc_curve <- roc(response = as.numeric(as.factor(cluster_test_labels_ae)), predictor = as.numeric(predictions))
  auc_value <- auc(roc_curve)
  
  # Store the performance metrics
  performance_metrics_ae <- rbind(performance_metrics_ae, data.frame(Cluster = k,
                                                               Accuracy = accuracy,
                                                               Recall = recall,
                                                               Precision = precision,
                                                               F1_Score = F1_score,
...
for (k in unique(clusters)) {
  ...
  # Train the SVM model
  svm_models[[k]] <- svm(x = cluster_training_data[, -13], y = cluster_training_labels, kernel = "radial", cost = 100, gamma = 0.1)
...
# 
# 
# Combine Cluster 2+3 and see prediction results


training_encoded_points_vae_12D_cluster_2_3 = training_encoded_points_vae_12D_with_clusters%>%
                                              filter(Cluster %in% c(2,3))

training_data_smote_with_clusters_2_3 =training_data_smote_with_clusters%>%
                                              filter(Cluster %in% c(2,3))

training_labels_cluster_2_3 = training_data_smote_with_clusters_2_3$DEFAULT_PAYMENT_NEXT_MONTH


testing_encoded_points_vae_12D_cluster_2_3 = testing_encoded_points_vae_12D_with_clusters%>%
                                              filter(Cluster %in% c(2,3))
testing_data_with_clusters_2_3 = testing_data_with_clusters%>%
                                filter(Cluster %in% c(2,3))
testing_labels_cluster_2_3 = testing_data_with_clusters_2_3$DEFAULT_PAYMENT_NEXT_MONTH



svm_model_vae_cluster_2_3 = svm(
    x = training_encoded_points_vae_12D_cluster_2_3[, -13],
    y = training_labels_cluster_2_3,
    type = "C-classification",
    kernel = "radial",
    cost = 100,
    gamma = 0.1,
...
summary(svm_model_vae_cluster_2_3)


svm_test_predict_cluster_2_3 = predict(svm_model_vae_cluster_2_3, testing_encoded_points_vae_12D_cluster_2_3[, -13])
svm_vae_accuracy_cluster_2_3 = mean(svm_test_predict_cluster_2_3==as.numeric(as.matrix(testing_data_with_clusters_2_3[,27])))

confusion_matrix_vae_cluster_2_3 = confusionMatrix(svm_test_predict_cluster_2_3, testing_data_with_clusters_2_3$DEFAULT_PAYMENT_NEXT_MONTH)

svm_metric_vae_cluster_2_3 = svm_metrics(confusion_matrix_vae_cluster_2_3)

roc_curve_vae_cluster_2_3 <- roc(response = testing_data_with_clusters_2_3$DEFAULT_PAYMENT_NEXT_MONTH, predictor = as.numeric(svm_test_predict_cluster_2_3))
AUC_vae_cluster_2_3 <- auc(roc_curve_vae_cluster_2_3)

svm_vae_accuracy_cluster_2_3
svm_metric_vae_cluster_2_3$recall
svm_metric_vae_cluster_2_3$precision
svm_metric_vae_cluster_2_3$F1_score
AUC_vae_cluster_2_3
