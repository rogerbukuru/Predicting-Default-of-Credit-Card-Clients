...
pca_reconstructed = pca_scores
reconstructed_original_scale = sweep(pca_reconstructed,2,sigma_x_continous_oversampled, "*")
reconstructed_original_scale = sweep(pca_reconstructed,2,mu_x_continous_oversampled, "+")

pca_mse = mean((as.matrix(train_X_continous_oversampled) - as.matrix(reconstructed_original_scale))^2)

autoencoder_stats = evaluate(autoencoder_model, train_X_continous_std_oversampled, train_X_continous_std_oversampled)

autoencoder_stats["loss"]
pca_mse

#c(1:10, 1:5)

#df <- data.frame(k = c(1:10, 1:5), mse = c(pca_mse, ae.mse), method = c(rep("pca", 10), rep("autoencoder", 5)))
