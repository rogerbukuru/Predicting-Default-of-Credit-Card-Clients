...
for(k in 1:12){
  train_aem = train_autoencoder(epochs = 100, code_size = k, training_data_x_std_smote)
  ...
  autoencoder_result[[k]]$autoencoder_model = train_aem$autoencoder_model
  autoencoder_result[[k]]$input_layer = train_aem$input_layer
  autoencoder_result[[k]]$encoder = train_aem$encoder
}
autoencoder_mse
...
for(k in 1:12){
 ...
 vae_result[[k]]$vae_model = train_vae_model$vae_model
 vae_result[[k]]$input_layer = train_vae_model$input_layer
 vae_result[[k]]$z = train_vae_model$z
}
...
wcss_pca <- numeric()
for (i in 1:10) {
  kmeans_result_pca <- kmeans(pca_scores[,14], centers = i, nstart = 25)
...
kmeans_result_pca <- kmeans(pca_scores[,14], centers = 3, nstart = 25)
...
s3d_pca = scatterplot3d(x = pca_scores$PC1, y = pca_scores$PC2, z = pca_scores$PC3, main = "Principal Components",
...
ggplot(risk_level_summary_pca, aes(x = Cluster, y = n, fill = Risk_Level)) +
  ...
  scale_fill_brewer(palette = "Set1", labels = c("High Risk", "Medium Risk", "Low Risk")) # Set custom labels for the legend
...
kmeans_result_ae <- kmeans(encoded_points_ae, centers = 3, nstart = 25)
 ...
 s3d_ae = scatterplot3d(x = encoded_points_ae$V1, y = encoded_points_ae$V2, z = encoded_points_ae$V3, main = "Autoencoder 3D Embedding", xlab = "Dim 1", ylab = "Dim 2", zlab = "Dim 3", pch = 19, color = point_colors_ae, cex.axis = 0.5)
# Add a legend
# Adjust the position based on your plot's scale
legend_position <- s3d_ae$xyz.convert(1.7, 1.0, -1.0)
...
kmeans_result_vae <- kmeans(encoded_points_vae, centers = 5, nstart = 25)


 cluster_colors_vae <- c("#0799BC", "#F98524", "#F1EA81", "darkgreen")  # Specify colors for each cluster
 point_colors_ae <- cluster_colors_vae[kmeans_result_vae$cluster]
 
 s3d_vae = scatterplot3d(x = encoded_points_vae$V1, y = encoded_points_vae$V2, z = encoded_points_vae$V3, main = "Variational Autoencoder 3D Embedding", xlab = "Dim 1", ylab = "Dim 2", zlab = "Dim 3", pch = 19, color = point_colors_ae, cex.axis = 0.5)
# Add a legend
# Adjust the position based on your plot's scale
legend_position <- s3d_vae$xyz.convert(1.7, 1.0, -1.0)
