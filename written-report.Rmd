...
# z = layer_concatenate(list(z_mean, z_log_var))%>%layer_lambda(f = function(arg){
#   z_mean <- arg[, 1:(latent_space_dim)]
#   z_log_var <- arg[, (latent_space_dim + 1):(2 * latent_space_dim)]
#   epsilon = k_random_normal(shape = c(k_shape(z_mean)[[1]]), mean = 0, stddev = 1)
#   z_mean + k_exp(z_log_var/2)*epsilon
# }, output_shape = c(latent_space_dim), name = "samplingLayer")

#samplingLayer = SamplingLayer$new()
#z = samplingLayer$call(list(z_mean, z_log_var))
...
# latent_inputs <- layer_input(shape = c(latent_space_dim), name= "latentLayer")
# decoder_outputs =
#   latent_inputs %>%
#   layer_dense(units = total_features/2 , activation = "relu", name="decoderHiddenL1")%>% #Hidden layer 1
#   layer_dense(units = total_features, activation = "relu", name = "decoderOutput") # Reconstructed/Output layer
# 
# decoder = keras_model(inputs = latent_inputs, outputs = decoder_outputs, name = "decoder")
# summary(decoder)
...
plot(vae)
visual_vae_latent_data(latent_space_dim,input_layer, z, x_train)
return(list(vae_model = vae_model))
}

visual_vae_latent_data = function(latent_space_dim,input_layer, z, x_train){
