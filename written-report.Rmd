...
s3d = scatterplot3d(x = pca_scores$PC1, y = pca_scores$PC2, z = pca_scores$PC3, main = "Principal Components",
              xlab = "PC1", ylab = "PC2", zlab = "PC3", pch = 19, color = point_colors_pca, cex.axis = 0.5)
legend_position <- s3d$xyz.convert(8, 1.0, -14.0)
...
ggplot(risk_level_summary_pca, aes(x = Cluster, y = n, fill = Risk_Level)) +
  ...
  theme_minimal() +
  labs(title = "Credit Risk Level Segments within Each Cluster - PCA",
       x = "Cluster",
       y = "No of customers",
       fill = "Credit Default Risk Level") +
...
## Autoencoders Customer Segements
...
encoded_points = ae_encoded_points(
...
wcss_ae <- numeric()
for (i in 1:10) {
  kmeans_result_ae <- kmeans(encoded_points, centers = i, nstart = 25)
...
kmeans_result_ae <- kmeans(encoded_points, centers = 3, nstart = 25)
 ...
 s3d = scatterplot3d(x = encoded_points$V1, y = encoded_points$V2, z = encoded_points$V3, main = "Autoencoder 3D Embedding", xlab = "Dim 1", ylab = "Dim 2", zlab = "Dim 3", pch = 19, color = point_colors_ae, cex.axis = 0.5)
# Add a legend
# Adjust the position based on your plot's scale
legend_position <- s3d$xyz.convert(1.7, 1.0, -1.0)
...
ggplot(risk_level_summary_ae, aes(x = Cluster, y = n, fill = Risk_Level)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(title = "Credit Risk Level Segments within Each Cluster - Autoencoder",
       x = "Cluster",
       y = "No of customers",
       fill = "Credit Default Risk Level") +
  scale_fill_brewer(palette = "Set1", labels = c("High", "Medium", "Low")) # Set custom labels for the legend
