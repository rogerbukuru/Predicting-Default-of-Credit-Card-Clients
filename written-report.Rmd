...
for (k in unique(clusters)) {
  ...
  cluster_data <- training_encoded_points_vae_12D_with_clusters[clusters == k, ]
  cluster_labels <- training_labels[clusters == k]
  cluster_test_labels <- testing_labels[test_data_clusters == k]
  
  # Train the SVM model
  svm_models[[k]] <- svm(x = cluster_data[, -13], y = cluster_labels, kernel = "radial", cost = 2, gamma = 0.1)
