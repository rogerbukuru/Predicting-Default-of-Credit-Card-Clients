...
credit_spending = credit_default_data %>%
                  select(LIMIT_BAL, BILL_AMT6) %>%
                  mutate(across(everything(), as.numeric)) %>%
                  mutate(`Total Outstanding` = if_else(BILL_AMT6 == 0, 0, LIMIT_BAL - BILL_AMT6)) %>%
                  ungroup()
                  
ggplot(credit_spending, aes(x = `Total Outstanding`)) +
  geom_histogram(binwidth = 50000, # Adjust binwidth based on your data's distribution and scale
                 fill = "blue", color = "black") +
  labs(title = "Histogram of Total Outstanding",
       x = "Total Outstanding",
       y = "# of Customers") +
  theme_minimal() 
...
outstanding_and_paid = data.frame(credit_spending$LIMIT_BAL, credit_spending$`Total Outstanding`, client_payments$`Total Payments`) |> 
  as.tibble()
...
```{r, echo= FALSE}
rm(list = ls())
file_path = "../Data/Assignment/student_data.csv"
online_retail_data = as_tibble(read_csv(file_path))
